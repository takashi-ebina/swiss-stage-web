...
**受け入れ条件**:
- [ ] 正規表現パターンで段級位フォーマットを検証（初段、2-9段、1-20級）
- [ ] `parse(String input)`メソッドで文字列からRankインスタンスを生成
- [ ] 不正なフォーマットで`IllegalArgumentException`をスロー
- [ ] level値が正しく変換される（初段→1、3段→3、1級→-1、3級→-3）
- [ ] `compareTo()`でlevel値降順の比較が可能
- [ ] 単体テストで10以上のテストケースをカバー
